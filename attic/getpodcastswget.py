...
def download(rss_feed_url, lang, download_folder, latest, relative, first, last, transcribeAsWell, sync, dryrun, image, save_meta, complete_n=None):
        ...
        if icon:
            ...
            if not os.path.exists(icon_path):
                if not dryrun:
                    ...
                    wget_command = f'wget -c -U "{user_agent}" -O "{icon_path}" "{icon}"'
                    print(f"[{folder_name}] Downloading podcast image...", file=sys.stderr)
                    if os.system(wget_command) == 0:
                        print(f"[{folder_name}] Podcast image downloaded: {icon_path}", file=sys.stderr)
                    else:
                        print(f"[{folder_name}] Failed to download podcast image.", file=sys.stderr)
                else:
                    print(f"[{folder_name}] Dryrun: Not downloading image.", file=sys.stderr)
            else:
    ...
    for latest_episode in episodes_to_download:
        ...
        if os.path.exists(mp3path):
            ...
            wget_command = f'wget -c -U "{user_agent}" -O "{mp3path}" "{media_url}"'
            
            print(f"[{folder_name}] Running command: {wget_command}", file=sys.stderr)
            ...
            if download_status == 0:
                ...
                print(f"[{folder_name}] Failed to download the episode. wget exit code: {download_status}", file=sys.stderr)

    # Sync only if new episodes were downloaded
    if sync and podcatch:
        print(f"[{folder_name}] Syncing to NAS")
        command = f"rsync --exclude='*.meta' -avz --progress {download_folder}/ mark@rpm17.local:/volume2/transpodcontent/content/{folder_name}"
        print(command)
        os.system(command)
    elif sync:
